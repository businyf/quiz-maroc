...
function splitIntoLines(text, wordsPerLine) {
  ...
  for (let i = 0; i < words.length; i += wordsPerLine) {
    lines.push(words.slice(i, i + wordsPerLine).reverse().join(" "))
  }
  return lines
}
...
export async function GET(request) {
  try {
    ...
    const subtitleLine = "جنسيتي حسب الذكاء الإصطناعي".split(" ").reverse().join(" ")
    const titleLine = titleAr.split(" ").reverse().join(" ")
    const descLines = splitIntoLines(desc, 5)
